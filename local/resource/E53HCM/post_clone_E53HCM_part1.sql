-- ************************************************************************************************************************
-- HR 9.2 Reconfigure Script for DEV - to be run after FT database (E53HCM) is refreshed (cloned) from another database
-- change history:
--    o) update 01/11/2015 after H&S release
-- ************************************************************************************************************************

-- * Note: Non-SSL version as DEV does not have SSL/TLS installed.
--
-- BEFORE STARTING:
--
...
UPDATE PSOPTIONS 
SET GUID = ' ', 
SYSTEMTYPE='DEV', SHORTNAME=(select name from v$database), LONGNAME=(select name from v$database)
/
...
UPDATE PS_HRS_APP_EMAIL 
  set EMAIL_ADDR = 'peoplesoft_applicant@hrisdev.ds.acc.co.nz'      -- Change UAT to DEV for E53HCM and SIT for E54HCM 
where EMAIL_ADDR <> ' ';
/

-- *****************
-- Process Scheduler - only PSUNX in DEV (Prod has PSUNX1 and PSUNX2)
-- *****************
...
set url = 'https://hris-dev.ds.acc.co.nz/psreports/ps',
opsys = '4',
uri_host = 'hris-dev.ds.acc.co.nz',
uri_port = '443',
uri_resource = 'SchedulerTransfer/ps',
uri_scheme = 'http',
uri_rpt='https://' || 'hris-dev.ds.acc.co.nz/psc/ps/EMPLOYEE/HRMS/c/CDM_RPT.CDM_RPT.GBL?Page=CDM_RPT_INDEX' || chr(38) || 'Action=U' || chr(38) || 'CDM_ID=',
...
update ps_cdm_dist_node 
set url = 'http://kdclea5053.ds.acc.co.nz:8000/psreports/ps',
opsys = '4',
uri_host = 'kdclea5053.ds.acc.co.nz',
uri_port = '8000',
uri_resource = 'SchedulerTransfer/ps',
uri_scheme = 'http',
uri_rpt='http://' || 'kdclea5053.ds.acc.co.nz:8000/psc/ps/EMPLOYEE/HRMS/c/CDM_RPT.CDM_RPT.GBL?Page=CDM_RPT_INDEX' || chr(38) || 'Action=U' || chr(38) || 'CDM_ID=',
cdm_dir_template = '%DBNAME%/%CURRDATE%/%REPORTID%'
...
update psurldefn 
set url = 'sftp://hrisftp:<password>@kdclea5053.ds.acc.co.nz/opt/oracle/ftpattachment/PAYSLIPS'
where url_id = 'GP_SS_PSLP_FTP'
/

update psurldefn 
set url = 'http://kdclea5053.ds.acc.co.nz:8000/psp/ps/EMPLOYEE'
where url_id = 'EMP_SERVLET'
/

update psurldefn 
set url = 'http://kdclea5053.ds.acc.co.nz:8000/psp/ps/'
...
UPDATE PSURLDEFN 
SET URL = 'devtest@hrisdev.ds.acc.co.nz'      -- Change UAT to DEV for E53HCM and SIT for E54HCM
...
UPDATE PSURLDEFN 
SET URL = 'peoplesoft@hrisdev.ds.acc.co.nz'      -- Change UAT to DEV for E53HCM and SIT for E54HCM
  , LASTUPDOPRID = 'REFRESH'
  , LASTUPDDTTM = SYSDATE
...
DELETE FROM PSAPMSGDSPSTAT WHERE MACHINENAME <> 'kdclea5053.ds.acc.co.nz' OR (APPSERVER_PATH <> '/opt/oracle/pt855/config/appserv/HR92DEV' AND MACHINENAME= 'kdclea5053.ds.acc.co.nz')
/

DELETE FROM PSAPMSGDOMSTAT WHERE MACHINENAME <> 'kdclea5053.ds.acc.co.nz' OR (APPSERVER_PATH <> '/opt/oracle/pt855/config/appserv/HR92DEV' AND MACHINENAME= 'kdclea5053.ds.acc.co.nz')
/

DELETE FROM PSAPMSGQUEUESET WHERE MACHINENAME <> 'kdclea5053.ds.acc.co.nz' OR (APPSERVER_PATH <> '/opt/oracle/pt855/config/appserv/HR92DEV' AND MACHINENAME= 'kdclea5053.ds.acc.co.nz')
/
...
update psgateway
set connurl = 'http://kdclea5053.ds.acc.co.nz:8000/PSIGW/PeopleSoftListeningConnector'
/
...
set URI_TEXT = 'http://kdclea5053.ds.acc.co.nz:8000/psc/ps/'
where MSGNODENAME='PSFT_HR'
and URI_TYPE = 'CN'
/

update PSNODEURITEXT
set URI_TEXT = 'http://kdclea5053.ds.acc.co.nz:8000/psp/ps/'
where MSGNODENAME='PSFT_HR'
and URI_TYPE = 'PL'
/
...
--/
--insert into PSWEBPROFNVP(webprofilename, propertyname, pt_propvalue, pt_lpropvalue) values('TEST', 'PSWEBSERVERNAME', ' ', 'hris-dev.ds.acc.co.nz')
--/
